...
const Test = (props: {name: string; test: TestCase}) => {
    ...
    return <div style={{lineHeight: 1}}>
        ...
        {passed == true && <h5 style={{marginTop: 0, marginBottom: 0, color: "green"}}>Passed</h5>}
        {passed == false && <h5 style={{marginTop: 0, marginBottom: 0, color: "red"}}>Failed</h5>}
    </div>;
}
...
root.render(
    <div>
        <Tests tests={{simple, serializingHeaders, responseHeaders, cookie, urlEncoded, multipartEncoded, numberResponse, stringResponse, nullResponse, trueResponse, falseResponse, readCookie, readBlob, badRequest, networkError}} />
    </div>,
);
